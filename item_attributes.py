...
# اختلافات تهجئة عربية شائعة حقيقية لنفس الكلمة (Benchmark حقيقي 2026-08-28:
# فاتورة تكتب "شوكولاتة" بينما القاعدة مسجّلة "شوكلاته" - نفس الكلمة، حرف
# مد "و" ساقط بمنتصف الكلمة، نمط شائع بكلمات معرّبة/مستوردة). كل قيمة (يمين)
# هي الشكل القانوني المعتمد بعد التطبيع - أي مفتاح (يسار) بالنص يُستبدل به.
# جدول بيانات صريح ومحافظ (استبدال كلمة كاملة، مو حروف منفردة) - **مو**
# قاعدة عامة "احذف أي و مكرر" (كانت راح تدمج كلمات مختلفة فعلاً بالغلط).
# يُضاف له حالات حقيقية جديدة مستقبلاً بدون لمس أي منطق ثاني بهذا الملف.
_SPELLING_ALIASES = {
    "شوكلاته": "شوكولاته",
}

# بدائل تهجئة/كتابة لنفس الماركة التجارية (Benchmark حقيقي 2026-08-28:
# فاتورة تكتب "7 دايز"، القاعدة مسجّلة "سفن دايز" - نفس الماركة "Seven Days"،
# صياغة مختلفة تماماً حرفياً فلا يمسكها أي تطبيع حروف عادي). جدول بيانات
# صريح وعام (مو حالة واحدة hardcoded) - كل قيمة (يمين) هي الشكل القانوني
# المعتمد؛ يُضاف له ماركات جديدة مستقبلاً كصف واحد بدون تعديل أي كود. **لا**
# تُستخدم وحدها كهوية صنف كافية - حجم/عدد/نوع تعبئة تبقى تُفحص كالعادة
# بـcheck_attribute_conflict، هذا بس يوسّع الاسترجاع (يدخل السباق).
_BRAND_ALIASES = {
    "7 دايز": "سفن دايز",
    "7 days": "سفن دايز",
    "seven days": "سفن دايز",
...
def _normalize_text(text: str) -> str:
    ...
    text = _apply_word_aliases(text)
    return text


# أطول العبارات أولاً - عشان بديل متعدد الكلمات (مثل "seven days") ينطبق
# قبل ما تفكّكه أي مطابقة جزئية أقصر بالخطأ
_ALL_ALIASES_BY_LEN = sorted(
    {**_SPELLING_ALIASES, **_BRAND_ALIASES}.items(), key=lambda kv: len(kv[0]), reverse=True
)


def _apply_word_aliases(text: str) -> str:
    """تمريرة استبدال أخيرة بجداول بيانات صريحة (_SPELLING_ALIASES/
    _BRAND_ALIASES) على النص بعد كل خطوات التطبيع - حدود كلمة (\\b) تمنع
    استبدال جزء من كلمة أطول بالخطأ. تشغيلها هنا (نهاية _normalize_text)
    يضمن استفادة كل مستهلكي normalized_text تلقائياً: تطابق الاسم الحرفي
    (_has_exact_normalized_name_match)، الاسترجاع الضبابي بالاسم
    (matching_engine._retrieve_candidate_hits)، وتخمين الماركة بالأسفل -
    "7 دايز" تتحول لـ"سفن دايز" *قبل* تفكيك الكلمات، فلا يعود أول رمز
    بالنص رقماً يوقف تخمين الماركة."""
    for variant, canonical in _ALL_ALIASES_BY_LEN:
